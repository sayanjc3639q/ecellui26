...
export const HeroSection: React.FC = () => {
  return (
    <section className="pt-40 pb-44 min-h-[88vh] flex items-center justify-center overflow-hidden relative border-b-4 border-black bg-white dark:bg-[#121418] text-center bg-[url('/hero-bg.png')] bg-cover bg-center bg-no-repeat">
      ...
      {/* 6. HERO CONTENT */}
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6 sm:space-y-8 relative z-20">
        <h1 className="gsap-hero-item font-display font-black text-4xl sm:text-7xl lg:text-8xl tracking-tight text-slate-900 dark:text-white leading-[1.08]">
          Innovation Starts with You
        </h1>
        <p className="gsap-hero-item font-body text-slate-700 dark:text-slate-200 text-sm sm:text-xl lg:text-2xl max-w-3xl mx-auto leading-relaxed px-2">
          We’re here to accelerate it. Build, launch, and scale your ideas with the finest minds in the ecosystem.
        </p>
        <div className="gsap-hero-item flex flex-wrap justify-center gap-4 sm:gap-6 pt-2 sm:pt-4">
          <Link href="/events">
            <MemphisButton variant="primary" className="flex items-center gap-2 px-6 py-3.5 sm:px-10 sm:py-4 text-sm sm:text-lg shadow-[3px_3px_0px_0px_#000] sm:shadow-[4px_4px_0px_0px_#000]">
              Events <ArrowRight className="w-4 h-4 sm:w-5 sm:h-5" />
            </MemphisButton>
          </Link>
          <Link href="/idea-pitching">
            <MemphisButton variant="outline" className="px-6 py-3.5 sm:px-10 sm:py-4 text-sm sm:text-lg bg-white/90 dark:bg-slate-900/90 backdrop-blur-sm shadow-[3px_3px_0px_0px_#000] sm:shadow-[4px_4px_0px_0px_#000]">
              Idea
            </MemphisButton>
          </Link>
        </div>
